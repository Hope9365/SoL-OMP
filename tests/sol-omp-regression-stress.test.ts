...
describe("SoL-OMP regression stress", () => {
	it("keeps EPR reducer routing on the configured Luna model under repeated calls", async () => {
		const root = await mkdtemp(join(tmpdir(), "sol-omp-epr-stress-"));
		try {
			for (let i = 0; i < 200; i++) {
				...
				const context = fakeContext(new FakeSessionManager([], `epr-stress-${i}`, root), {
					...
					modelRegistry: {
						...
						getApiKey: async (model: Model<string>) => { authModel = model; return "test-key"; },
	...
	it("fails open without calling a model when the configured reducer model is unavailable", async () => {
		const root = await mkdtemp(join(tmpdir(), "sol-omp-epr-missing-stress-"));
		try {
			for (let i = 0; i < 100; i++) {
