...
describe("SoL-OMP config", () => {
	...
	it("ignores the project config when Pi has not trusted the project", () => {
		...
		mkdirSync(join(cwd, CONFIG_DIR_NAME));
		writeFileSync(join(cwd, CONFIG_DIR_NAME, "sol-omp.json"), JSON.stringify({ version: 1, actionFusion: true }));

		expect(loadSolOmpConfig(cwd, agentDir, false)).toEqual({ ...DEFAULT_CONFIG, observationPack: true });
	});
	...
	it("normalizes surrounding whitespace in the EPR reducer route", () => {
		...
		expect(loadSolOmpConfig(cwd, agentDir, true)).toMatchObject({
			evidencePreservingReducerProvider: "test-provider",
			evidencePreservingReducerModel: "test-reducer-model",
		});
	});
