...
const SCRIPT = join(process.cwd(), "scripts/check-sol-omp-config.mjs");

let directory: string;

beforeEach(() => {
	directory = mkdtempSync(join(tmpdir(), "sol-omp-config-preflight-"));
});
...
function writeConfig(value: unknown): string {
	const path = join(directory, "sol-omp.json");
...
describe("SoL-OMP configuration preflight", () => {
