 ...
 * provider/model selected by the top-level SoL-OMP config, and accepts the
 ...
 *
 * The top-level SoL-OMP config enables this mechanism. Provider selection and
...
import { formatSavingsBytes, showSolOmpSavings } from "../../tui.ts";
...
import { callReducer, type ProviderResult, type ReducerComplete } from "./provider.ts";
...
export type EvidencePreservingReducerOptions = ReducerConfigOptions & { readonly complete?: ReducerComplete };
...
export async function reduceToolResult(
	...
	try {
		provider = await callReducer(config, command, event.isError, archive, body, context, complete);
	} catch (error) {
	...
	showSolOmpSavings(context, "Evidence-Preserving Reducer", formatSavingsBytes(archive.bytes - receiptBytes));
	return {
...
export function createEvidencePreservingReducerExtension(options: EvidencePreservingReducerOptions = {}): ExtensionFactory {
	return (pi: ExtensionAPI) => {
		...
		pi.on("tool_result", (event, context) => {
			...
			return reduceToolResult(state.journal, state.config, event, context, options.complete);
		});
	};
}
