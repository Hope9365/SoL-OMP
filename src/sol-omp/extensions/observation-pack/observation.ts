...
// ponytail: Windows lacks O_NOFOLLOW; lstat catches stable links, use native no-follow handles if hostile concurrent writers matter.
async function rejectObjectSymlink(path: string): Promise<void> {
	if (!(await lstat(path)).isSymbolicLink()) return;
	throw Object.assign(new Error("Stored observation is a symlink"), { code: "ELOOP" });
}
...
/**
 * Archived payloads live under SoL-OMP's session-derived runtime root.
 *
...
export function createObservation(message: ToolResultMessage, runtimeRoot: string): Observation | undefined {
	...
	if (!runtimeRoot) throw new Error("Persistent SoL-OMP runtime directory is unavailable");
...
export async function ensureStored(observation: Observation): Promise<void> {
	...
	try {
		...
		await rejectObjectSymlink(observation.filePath);
...
export async function readRecallChunk(
	...
	await rejectObjectSymlink(path);
