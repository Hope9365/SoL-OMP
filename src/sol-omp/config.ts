...
export function findConfigPath(
	...
	const globalPath = join(agentDir, "sol-omp.json");
	return existsSync(globalPath) ? globalPath : undefined;
}

export function loadSolOmpConfig(
	cwd = process.cwd(),
	agentDir = getAgentDir(),
	allowProjectConfig = false,
): SolOmpConfig {
	...
	try {
		...
		throw new Error(`Unable to read SoL-OMP config ${path}: ${reason}`);
	}

	if (typeof parsed !== "object" || parsed === null || Array.isArray(parsed)) {
		throw new Error(`SoL-OMP config must be a JSON object: ${path}`);
	}

	const record = parsed as Record<string, unknown>;
	for (const key of Object.keys(record)) {
		if (!CONFIG_KEYS.has(key)) throw new Error(`Unknown SoL-OMP config key: ${key}`);
	}
	if (record.version !== 1) throw new Error(`SoL-OMP config version must be 1: ${path}`);

	for (const key of FEATURE_KEYS) {
		if (record[key] !== undefined && typeof record[key] !== "boolean") {
			throw new Error(`SoL-OMP config ${key} must be boolean: ${path}`);
		}
	}
	...
	if (
		...
		throw new Error(`SoL-OMP config cacheWriteReadRatio must be a finite non-negative number: ${path}`);
	}
...
function stringConfigValue(
	...
	if (typeof value !== "string" || value.trim().length === 0) {
		throw new Error(`SoL-OMP config ${key} must be a non-empty string: ${path}`);
	}
	return value.trim();
}
